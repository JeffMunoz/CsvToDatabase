...
public class Main {
    ...
    public static void main(String[] args) throws IOException, CsvValidationException {
        ...
        JsonReader bookJread = new JsonReader(new FileReader("src/Data/bookstore_report2.json"));
        Book[] books = gson.fromJson(bookJread, Book[].class);

        // The var allows for any object to be used
        for (var element : authors) {
            System.out.println(element.getName());
        }

        BookStoreDB db = new BookStoreDB();
        db.insertAuthor(authors);
        db.insertBook(books);
    }
}
